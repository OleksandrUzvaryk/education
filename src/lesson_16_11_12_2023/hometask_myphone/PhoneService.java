...
public class PhoneService {
    ...
    // 5. удалить какую-то смску
        ...
        if(smsNumber>=0 && smsNumber<10){
            phone.getSimCard().getSms()[smsNumber] = null; return true;}
        else {System.out.println("Message with that number not exist "); return false;
            }
        }
        public boolean deleteSmsMessageSim2(Phone phone, int smsNumber){
        if (smsNumber >=0 && smsNumber <10){
            phone.getSimCardTwo().getSms()[smsNumber] = null; return true;}
        else {System.out.println("Message with that number not exist "); return false;}
        }
